...
# XGBoost
RUN cd /tmp && \
    git clone https://github.com/piaolaidelangman/xgboost-spark-sgx && \
    cd xgboost-spark-sgx && \
    /opt/apache-maven-3.6.3/bin/mvn -T 16 -DskipTests=true clean package && \
    mv target/xgboostsparksgx-1.0-SNAPSHOT.jar ${SPARK_HOME}/jars
# hive
RUN cd /opt/src && \
...
FROM occlum/occlum:0.27.1-ubuntu20.04 as ppml
...
RUN cd /opt && \
    ...
    ./download-bigdl.sh && \
    rm bigdl*.zip && \
    mv ${SPARK_HOME}/jars/xgboostsparksgx-1.0-SNAPSHOT.jar $BIGDL_HOME/jars

# Prepare Attestation
ADD ./build_content.sh /root/demos/remote_attestation/init_ra_flow/
ADD ./run_attestation_server.sh /root/demos/remote_attestation/init_ra_flow/
RUN cd /root/demos/remote_attestation/init_ra_flow/ && \
    bash build_content.sh build_ra_tls
# Prepare Tpc-H
